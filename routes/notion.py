...
async def notion_webhook(request: Request):
    ...
    secret = os.getenv("NOTION_WEBHOOK_SECRET")
    if not secret:
        print("[NOTION][ERROR] NOTION_WEBHOOK_SECRET not set - rejecting webhook (fail-closed).")
...
def create_todo_page(
    *,
    ...
    """
    Create a new page in the Todo database.

    Fields map to the same properties parse_todo reads:
      Name (title), Date Due (date), Date (date),
      Status (status), Text (rich_text), image (files).
    """
    database_id = _require_env("NOTION_TODO_ID")

    if not name or not str(name).strip():
        raise ValueError("name is required")

    properties: dict[str, Any] = {
        "Name": {"title": [{"text": {"content": str(name).strip()}}]},
    }

    if date_due:
        properties["Date Due"] = {"date": {"start": date_due}}
    if date:
    ...
    with httpx.Client(timeout=timeout) as client:
        for attempt in range(1, max_retries + 1):
            try:
                response = client.post(url, headers=_notion_headers(), json=payload)
                response.raise_for_status()
                return response.json()
            except httpx.ReadTimeout:
                if attempt == max_retries:
...
@router.post("/todos", dependencies=[Depends(_require_test_token)])
async def create_todo(request: Request):
    """
    Create a Todo item in the Notion Todo database.

    Auth: Bearer TEST_TOKEN (same as /test endpoints).

    Body (JSON):
      {
        "name":      "Task title",           # required
        "date_due":  "2026-09-15",           # optional, YYYY-MM-DD
        "date":      "2026-09-08",           # optional, YYYY-MM-DD
